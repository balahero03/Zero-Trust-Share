...
import { SimpleFileDownload } from '@/components/SimpleFileDownload';
...
export default function SharePage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Extract key from URL hash if present (for legacy support)
      const hash = window.location.hash;
      if (hash.startsWith('#key=')) {
        const extractedKey = decodeURIComponent(hash.substring(5));
        setKey(extractedKey);
      } else if (hash.startsWith('#salt=')) {
        const extractedSalt = decodeURIComponent(hash.substring(6));
        setKey(extractedSalt);
      }
    }
  }, [params.fileId]);
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Enter the password provided by the sender to securely download your file.
            <br />
            <span className="text-electric-blue font-semibold">Your file is encrypted and secure.</span>
          </p>
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm text-gray-400">
            🔒 Your file is decrypted entirely in your browser. We cannot access your files even if we wanted to.
            <br />
            All decryption happens locally on your device for maximum security.
          </p>
        </div>
      </main>
    </div>
